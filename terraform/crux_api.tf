...
# Adding Response Plays to Services
  provisioner "local-exec" {
    command = "/bin/bash ../crux-api/addResponsePlay.sh ${var.PAGERDUTY_TOKEN} ${pagerduty_service.tech_service_1a.id} ${pagerduty_response_play.majorIncident.id}"
  }

  provisioner "local-exec" {
    command = "/bin/bash ../crux-api/addResponsePlay.sh ${var.PAGERDUTY_TOKEN} ${pagerduty_service.tech_service_1c.id} ${pagerduty_response_play.majorIncident.id}"
  }

  provisioner "local-exec" {
    command = "/bin/bash ../crux-api/addResponsePlay.sh ${var.PAGERDUTY_TOKEN} ${pagerduty_service.tech_service_4.id} ${pagerduty_response_play.majorIncident.id}"
  }
